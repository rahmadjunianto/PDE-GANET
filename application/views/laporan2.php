...
$tbl2 = '<table border="0" style="text-align:center; font-size:10px;">
		...
		';
				...
				$tbl2 = $tbl2.'<table border="1px" cellpadding="2" cellspacing="0">
					...
					$q=$this->db->query( 
					"SELECT ms.id_masalah,'' nomor,ms.tgl_lapor,ins.`nm_ins`,ms.`nama_pelapor`,ms.`keterangan`,us.`nama`,'' tgl_repair,'' masalah,'' solusi,'' team
FROM tb_masalah ms
...
UNION ALL 
SELECT ms.id_masalah,dt.nomor,'','','','','',dt.`tgl_repair`,dt.`masalah`,dt.`solusi`,dt.`team`
FROM tb_masalah ms
JOIN tb_instansi ins ON ms.`id_ins`=ins.`id_ins`
JOIN tb_user us ON ms.`penerima`=us.`id`
JOIN tb_masalah_dtl dt ON ms.`id_masalah`=dt.`id_masalah`
WHERE DATE_FORMAT(ms.tgl_lapor,'%Y-%m-%d')  BETWEEN  '".$tgl1."' AND '".$tgl2."'  
ORDER BY id_masalah,nomor")->result();
					foreach ($q as $c ) {
						$tbl2 .= '<tr style ="font-size:10px;">
													<td align="center" style="width:50px">'.(($c->nomor!='')?'':$i++).'</td>
													<td>'.$c->tgl_lapor.'</td>
													<td ></td>
													<td >'.$c->nm_ins.'</td>
